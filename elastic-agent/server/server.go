...
var (
	...
	ErrApplicationAlreadyRegistered = errors.New("application already registered")
	...
	ErrApplicationStopTimedOut = errors.New("application stopping timed out")
	// ErrActionTimedOut returned on PerformAction when the action timed out.
	ErrActionTimedOut = errors.New("application action timed out")
	...
	ErrActionCancelled = errors.New("application action cancelled")
)
...
func New(listenAddr string, handler Handler) (*Server, error) {
	...
	return &Server{
		...
		checkInMinTimeout:     control.CheckinMinimumTimeout + CheckinMinimumTimeoutGracePeriod,
	}, nil
}

// Run runs the GRPC control endpoint and accepts new connections.
func (s *Server) Run(ctx context.Context) error {
	...
	lis, err := listener.New(s.listenAddr, 6789)
	if err != nil {
		return err
	}
	s.listener = lis
	...
	go func() {
		...
		if err != nil {
			log.Error().Err(err).Msg("error listening for GRPC")
		}
	}()
...
func (s *Server) Register(app interface{}, config string) (*ApplicationState, error) {
	...
	appState := &ApplicationState{
		...
		statusConfigIdx:   control.InitialConfigIdx,
...
func (s *Server) Checkin(server proto.Control_CheckinServer) error {
	...
	case <-time.After(InitialCheckinTimeout):
		// close connection
		log.Debug().Msg("check-in stream never sent initial observed message; closing connection")
		return nil
	}
	if !ok {
		// close connection
	...
	if !ok {
		...
		log.Debug().Msg("check-in stream sent an invalid token; closing connection")
	...
	if appState.checkinDone != nil {
		...
		log.Debug().Msg("check-in stream already exists for application; closing connection")
	...
	if !appState.checkinConn {
		...
		appState.checkinLock.Unlock()
		log.Debug().Msg("check-in stream cannot connect, application is being destroyed; closing connection")
		...
		for {
			...
			if err != nil {
				if reportableErr(err) {
					log.Debug().Err(err).Msg("check-in stream failed to send expected state")
				}
				return
			}
		}
	...
	go func() {
		for {
			...
			if err != nil {
				if reportableErr(err) {
					log.Debug().Err(err).Msg("check-in stream failed to receive data")
				}
				close(recvDone)
				return
			}
...
func (s *Server) Actions(server proto.Control_ActionsServer) error {
	...
	case <-time.After(InitialCheckinTimeout):
		// close connection
		log.Debug().Msg("actions stream never sent initial response message; closing connection")
		return nil
	}
	if !ok {
		// close connection
	...
	if firstResp.Id != control.ActionResponseInitID {
		// close connection
		log.Debug().Msg("actions stream first response message must be an init message; closing connection")
	...
	if !ok {
		// no application with token; close connection
		log.Debug().Msg("actions stream sent an invalid token; closing connection")
		return status.Error(codes.PermissionDenied, "invalid token")
	}
	...
	if appState.actionsDone != nil {
		...
		log.Debug().Msg("actions stream already exists for application; closing connection")
		return status.Error(codes.AlreadyExists, "application already connected")
	}
	if !appState.actionsConn {
		// application is being destroyed cannot reconnect; close connection
		appState.actionsLock.Unlock()
		log.Debug().Msg("actions stream cannot connect, application is being destroyed; closing connection")
		return status.Error(codes.Unavailable, "application cannot connect being destroyed")
	}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				if reportableErr(err) {
					log.Debug().Err(err).Str("id", pending.id).Msg("failed to send pending action")
				}
				return
			}
	...
	go func() {
		for {
			...
			if err != nil {
				if reportableErr(err) {
					log.Debug().Err(err).Msg("actions stream failed to receive data")
				}
				close(recvDone)
				return
			}
			...
			if !ok {
				...
				log.Debug().Err(err).Str("action", response.Id).Msg("actions stream received an unknown action")
				appState.actionsLock.Unlock()
				continue
			}
...
// GetAgentConn returns the connection information back to the agent.
func (as *ApplicationState) GetAgentConn() *proto.Agent {
	return &proto.Agent{
